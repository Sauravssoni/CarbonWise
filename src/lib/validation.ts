...
export const ReductionTargetSchema = z.union([
  z.literal(5),
  z.literal(10),
  z.literal(20),
]);
...
export const FootprintSchema = z.object({
  ...
  distancePerDayKm: z.number().min(0, 'Distance must be at least 0').max(1000, 'Distance cannot exceed 1000 km/day'),
  carpool: z.boolean(),
  flightsThisMonth: z.number().int().min(0, 'Flights must be at least 0').max(100, 'Flights cannot exceed 100/month').optional().default(0),

  // Home Energy Step
  electricityKwhMonthly: z.number().min(0, 'Electricity must be at least 0').max(50000, 'Electricity use is unreasonably high'),
  acUsage: ACUsageLevelSchema,
  householdSize: z.number().int().min(1, 'Household size must be at least 1').max(20, 'Household size cannot exceed 20'),
  ...
  // Consumption Step
  onlineOrdersMonthly: z.number().int().min(0, 'Orders must be at least 0').max(500, 'Online orders cannot exceed 500/month'),
  clothingPurchasesMonthly: z.number().int().min(0, 'Clothing purchases must be at least 0').max(100, 'Clothing purchases cannot exceed 100/month'),
