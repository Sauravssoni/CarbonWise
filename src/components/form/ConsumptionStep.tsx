...
const RECYCLE_OPTIONS: { val: FrequencyLevel; label: string; description: string; icon: string }[] =
  [
    {
      val: 'Rare',
      label: 'Rare / Mixed Garbage',
      description: 'Combine all items into standard waste bins',
      icon: '🗑️',
    },
    {
      val: 'Sometimes',
      label: 'Active Sorting',
      description: 'Recycle standard drink packages, cans, and cartons',
      icon: '♻️',
    },
    {
      val: 'Often',
      label: 'Zero Waste Driven',
      description: 'Composting, garment repairs, and mindful buying habits',
      icon: '🛠️',
    },
  ];
    ...
    <fieldset className="space-y-6 animate-fade-in border-none p-0 m-0">
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-4 items-center">
          ...
          <input
            ...
            value={currentOrders}
            onChange={(e) =>
              onChange({ onlineOrdersMonthly: Math.min(500, Math.max(0, Number(e.target.value))) })
            }
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-4 items-center">
          ...
          <input
            ...
            onChange={(e) =>
              onChange({
                clothingPurchasesMonthly: Math.min(100, Math.max(0, Number(e.target.value))),
              })
            }
            className="w-24 text-center border border-slate-200 rounded-xl px-2 py-1.5 text-xs bg-white text-slate-800 font-mono focus:outline-none focus:ring-1 focus:ring-emerald-500"
          />
        ...
        <p className="text-xs text-slate-400 uppercase tracking-wide">
          Adding a standard cotton outfit carries ~8 kg direct CO2e output.
        </p>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div
          ...
          role="radiogroup"
          aria-label="Select recycling tendency"
        >
          {RECYCLE_OPTIONS.map((opt) => {
            ...
            return (
              ...
              >
                ...
                <div className="text-xs text-slate-400 mt-2 flex-grow leading-relaxed">
                  {opt.description}
                </div>
              </button>
            );
