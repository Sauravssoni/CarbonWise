...
const AC_OPTIONS: { val: ACUsageLevel; label: string; details: string; icon: string }[] = [
  { val: 'None', label: 'No AC / Off', details: 'No active electrical cooling used', icon: '❄️ Off' },
  { val: 'Low', label: 'Low Usage', details: 'Eco mode or under 2 hours/day', icon: '🍃 Low' },
  { val: 'Medium', label: 'Medium Usage', details: 'Normal cooling cycle (2-6 hours/day)', icon: '💨 Med' },
  { val: 'High', label: 'Heavy Usage', details: 'Extensive running (over 6 hours/day)', icon: '⚡ High' },
];
...
export default function EnergyStep({ data, onChange }: EnergyStepProps) {
  ...
  const handleBillChange = (val: number) => {
    setRawBillValue(val);
    const convertedKwh = Math.round(val / 0.20);
    ...
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-slate-400 text-xs mt-1">
          Grid energy consumption remains a principal generator of emissions. Shared homes divide this impact.
        </p>
      </div>

      {/* Input Toggle (Direct vs Bill) */}
      <div className="space-y-3">
        <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider">Monthly Power Account Input</label>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-4 items-center">
            ...
            <input
              ...
              max="50000"
              value={currentKwh}
              onChange={(e) => onChange({ electricityKwhMonthly: Math.min(50000, Math.max(0, Number(e.target.value))) })}
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-4 items-center">
            ...
            <input
              ...
              max="5000"
              value={rawBillValue}
              onChange={(e) => handleBillChange(Math.min(5000, Math.max(0, Number(e.target.value))))}
              className="w-24 text-center border border-slate-200 rounded-xl px-2 py-1.5 text-xs bg-white text-slate-800 font-mono focus:outline-none focus:ring-1 focus:ring-emerald-500"
            />
          </div>
      ...
      {/* Household sizes */}
      <div className="space-y-3">
        <label htmlFor="household-input" className="block text-xs font-bold text-slate-600 uppercase tracking-wider">
        ...
        <div className="flex items-center gap-2.5 bg-slate-50 border border-slate-200 rounded-xl p-3 max-w-xs">
          <input
            ...
            max="20"
            value={currentSize}
            onChange={(e) => onChange({ householdSize: Math.min(20, Math.max(1, Math.round(Number(e.target.value)))) })}
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3" role="radiogroup" aria-label="Select AC level">
          {AC_OPTIONS.map((opt) => {
